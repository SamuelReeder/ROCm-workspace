...
- **Fix Policy**: Only apply **trivial clang-tidy auto-fixes** (e.g., formatting, simple naming corrections). Any more complex errors (compilation failures, test failures, logic issues, etc.) must be reported immediately to the caller with a concise description and trimmed error message. Do not modify source files or CMake files for non-trivial issues.
