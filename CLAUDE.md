...
- Bootstrap clones shallow top-level repositories by default and reuses existing registry checkouts as Git object references; use `--full-history` for complete history and `--submodules` only when recursive submodule checkout is needed.
